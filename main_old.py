...
from backup_manager import startup_sequence, run_health_check
...
try:
    from email_bridge import EmailBridge, get_email_bridge, detect_email_command
...
rollback = None   # started on-demand only, not at startup
...
income_mgr = None   # started on-demand only, not at startup
...
def _update_working_memory_after_response(
    ...
    """Update working memory to reflect what just happened in this turn."""
    try:
        from working_memory import update

        # Detect open threads — sentences where Hayeong committed to something
        try:
            from commitment_manager import _COMMITMENT_PHRASES
            commitment_phrases = _COMMITMENT_PHRASES
        except Exception:
            commitment_phrases = ["i'll", "i will", "i'll get", "let me", "i can do that"]
        ...
        for phrase in commitment_phrases:
            if phrase in lower_response:
                for sentence in response.replace("!", ".").replace("?", ".").split("."):
                    if phrase in sentence.lower():
                        open_thread = sentence.strip()[:100]
                        break
                if open_thread:
                    break

        # Topic from intent or first few words of message
        topic = intent if intent not in ("conversation", "general", "") else ""
        if not topic:
            words = user_input.split()
            topic = " ".join(words[:4]) if len(words) > 2 else user_input

        # Emotional tone
        tone = "neutral"
        text = user_input.lower()
        if any(w in text for w in ["excited", "happy", "great", "awesome", "love"]):
            tone = "positive"
        elif any(w in text for w in ["frustrated", "annoyed", "broken", "wrong", "ugh"]):
            tone = "frustrated"
        elif any(w in text for w in ["curious", "wondering", "interesting", "how does"]):
            tone = "curious"
        ...
        update(
            ...
            james_intent=intent,
            hayeong_thought=response[:150] if response else "",
        )
    except Exception as e:
        print(f"[working_memory] Update failed: {e}")
...
_TOOL_TRIGGERS = [
    ...
    "google", "news about", "current price", "what happened",
    # Email
    "check my email", "send an email", "reply to", "email",
    # Image
    "generate an image", "make an image", "draw", "create an image",
    # Minecraft — explicit action requests only, not topic mentions
    # "mine"/"build"/"craft" removed: talking ABOUT Minecraft ≠ requesting an action
    "join the minecraft server", "start minecraft", "load minecraft",
    "connect to the server", "go to minecraft", "open minecraft",
    "go to base",
    # Tasks
    "add a task", "remind me", "show my tasks",
    # Apps
    "open ", "start ", "launch ",
    # Vision
    "look at my screen", "what do you see", "screenshot",
]

# Safe conversational contexts — never trigger tool-decision even if a trigger substring matches
_SAFE_CONTEXTS = frozenset(["offline", "maintenance"])

def _needs_tool_decision(user_input: str) -> bool:
    """
    Fast pre-check — does this message clearly need a tool?
    Runs in microseconds with no LLM call. Returns True only for
    explicit tool requests so the 14b is never called for small talk.
    """
    text = user_input.lower().strip()
    if any(ctx in text for ctx in _SAFE_CONTEXTS):
        return False
    for trigger in _TOOL_TRIGGERS:
        if trigger in text:
            return True
    return False
    ...
    def _stream(model: str):
        try:
            return requests.post(
                COMMUNICATION_URL,
                json={"model": model, "messages": messages, "stream": True, "keep_alive": -1, "options": {"num_ctx": 16384}},
                stream=True,
                timeout=60,
            )
        except Exception as e:
...
def main(text_mode: bool = False, brain_mode: bool = False):
    ...
    # ── TEMPORARY: Clear stale task state on startup ──
    # Temporary fix while wake assessment is being improved.
    # Long-term: wake assessment reads accurate offline time and decides what
    # to keep vs drop. Do not remove this comment when removing the clear.
    try:
        ...
        print("[startup] Task state cleared for clean session.")
    except Exception as _tsc_err:
        print(f"[startup] Could not clear task state: {_tsc_err}")

    try:
        from working_memory import clear as _clear_wm, update as _update_wm
        _clear_wm()
        _update_wm(topic="", tone="neutral")
        print("[startup] Working memory cleared.")
    except Exception as _wm_err:
        print(f"[startup] Could not clear working memory: {_wm_err}")
    ...
    # ── Start task agent — executes plans from the reasoning loop ──
    try:
        from task_agent import start_task_agent
        start_task_agent()
        print("   [TaskAgent] Started.")
    except Exception as _ta_err:
        print(f"   [TaskAgent] Failed to start: {_ta_err}")

    # ── Warm task agent model so first execution isn't cold ──
    try:
        ...
        print("   [TaskAgent] Task model warmed.")
    except Exception:
        pass
    ...
    email  = get_email_bridge() if EMAIL_AVAILABLE        else None
    ...
    # ── Email monitor — started on-demand only (not auto-started) ──
    email_monitor = None
    ...
    for _url, _model, _label in [
        (COMMUNICATION_URL, COMMUNICATION_MODEL, "communication (llama3.2)"),
        (REASONING_URL,     REASONING_MODEL,     "reasoning (deepseek-r1)"),
    ]:
        try:
    ...
    while True:
        ...
        # Fast keyword pre-check gates the 14b call — no LLM needed for this.
        # Only invoke decide_action() when the message clearly requests a tool.
        if _needs_tool_decision(user_input) and not is_wrap_up(user_input):
            decision = decide_action(user_input, memory, model=selected_model, snapshot=_snapshot)
        else:
            decision = {"action": "none"}
            print("   [Decision] skipped — conversational turn")
        ...
        # ── Action approval — verify only truly irreversible actions ──
        # Reversible actions (Minecraft, status, script launching) proceed directly
        # once _needs_tool_decision() + decide_action() both approved them.
        # A second LLM pass only fires for actions that can't be undone.
        _REQUIRES_VERIFICATION = {"email_send", "file_delete", "financial"}
        _constraints = []

        if action != "none":
            if action in _REQUIRES_VERIFICATION:
                ...
                print(f"   [Action] approved — {action} is reversible, no verification needed")
                if _snapshot:
                    _constraints = _snapshot.get("active_constraints", [])
        ...
        if action != "none":
            if _loader and _loader.handles(action):
                ...
                _cap_context = {
                    ...
                    "email_address": get_email_bridge().to_address if EMAIL_AVAILABLE else None,
        ...
        _update_working_memory_after_response(user_input, ai_response, _route_intent)
