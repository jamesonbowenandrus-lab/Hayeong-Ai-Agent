...
Rules:
  - If James mentions emailing a report or research → web_search with delivery=document
  - If the request is purely conversational → none
  - Only pick an action if you are CERTAIN it is needed
  - When in doubt → none
"""

def decide_action(user_input: str, memory: list, model: str = None) -> dict:
    """
    ...
    # Build a compact conversation context — last 4 turns only
...
def main(text_mode: bool = False):
    ...
    while True:
        ...
        decision = decide_action(user_input, memory, model=selected_model)
