...
# Approve/deny self-generated capabilities
...
def stream_response_and_speak(system_prompt: str, memory: list, emotion: str = "neutral") -> str:
    ...
    response = _stream(PRIMARY_MODEL) or _stream(FALLBACK_MODEL)
    if response is None:
    ...
    for line in response.iter_lines():
        ...
        if buffer_text and buffer_text[-1] in SENTENCE_ENDS:
            if len(buffer_text.split()) >= 3:
                sentence_queue.put(buffer_text)
                print(f"Hayeong: {buffer_text}")
                token_buffer = []
...
def main():
    ...
    # She opens her outside line herself. She doesn't wait to be told.
    print("\n📡 Starting Discord bridge...")
    result = procs.start("discord")
    print(f"   {result}")
    if procs.is_running("discord"):
        procs.monitor_and_restart("discord")  # she'll notice if it crashes

    print("\n✅ Hayeong is ready. Say her name to wake her up.")
    ...
    while True:
        ...
        # ── Exit commands ──
        if user_input.lower().strip() in ["exit", "shut down", "goodbye", "shut down hayeong"]:
            speak("Talk to you later.", emotion="neutral")
            save_memory(memory)
        ...
        # ── Capability command? ──
        # Check before sending to AI — these are direct actions, not conversation.
        action, cap = detect_capability_command(user_input)
        if action and cap:
            response_text = CAPABILITY_RESPONSES.get((action, cap), f"{action}ing {cap}.")
            print(f"\nHayeong: {response_text}")
            speak(response_text, emotion="neutral")
            ...
            # Still log the exchange so she remembers
        ...
        if smm:
            ...
            if approve_match or deny_match:
                ...
                if ok:
                    resp = f"{'Activated' if approved else 'Disabled'}: {filename}."
                else:
                    resp = f"Couldn't find '{filename}' in pending capabilities."
                print(f"\nHayeong: {resp}")
        ...
        if passphrase_attempt and session.trust_level < 2:
            ...
            if verified:
                speak("Hey, it's you. Good.", emotion="warm")
                continue
        ...
        if session.is_suspicious():
            ...
            if suspicion_response:
                ...
                speak(suspicion_response, emotion="guarded")
        ...
        ai_response = stream_response_and_speak(system_prompt, memory, emotion=current_emotion)
