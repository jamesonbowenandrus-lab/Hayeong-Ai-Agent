            ...
            if _delivery_mode == "document" and n_results > 0:
                ...
                if EMAIL_AVAILABLE:
                    try:
                        ...
                        _doc_emailed = email_result if isinstance(email_result, bool) else email_result.get("success", False)
                        if _doc_emailed:
                ...
                _doc_note = (
                    f"You emailed James the full research document. "
                    if _doc_emailed else
                    f"You saved the full research document to this exact path: {doc_path} — "
                    f"tell James this path if he asks where it is. "
                )
                _web_context = (
                    searcher.format_for_context(query, data) +
                    f"\n\n[DOCUMENT DELIVERY — IMPORTANT]: {_doc_note}"
                    "Now give James ONLY a 2-3 sentence personal take on what stands out most "
                    "from the results — your honest impression, not a summary of everything. "
                    "Then in one sentence mention you've saved/sent the full breakdown. "
                    "Do NOT list specs, do NOT use headers or bullets, just talk naturally."
                )
            else:
