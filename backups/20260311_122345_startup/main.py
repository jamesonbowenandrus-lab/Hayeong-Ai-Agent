...
def stream_response_and_speak(system_prompt: str, memory: list, emotion: str = "neutral", text_mode: bool = False) -> str:
...
def main(text_mode: bool = False):
    ...
    while True:
        ...
        # ── Task commands ──
        if tasks:
            task_cmd, task_remainder = detect_task_command(user_input)
            if task_cmd == "show":
                ...
                print()
                continue
            elif task_cmd == "add" and task_remainder:
                kwargs   = parse_task_from_text(task_remainder, origin="james")
        ...
        if email:
            ...
            if email_cmd == "send_summary":
                ...
                msg_text = re.sub(
                    r'(email me|send me a message|notify me|send me a note|send me a test email|send an email|send me an email)\s*',
        ...
        ai_response = stream_response_and_speak(system_prompt, memory, emotion=current_emotion, text_mode=text_mode)
