...
def main(text_mode: bool = False):
    ...
    while True:
        ...
        # One call classifies the message before anything else runs.
        # Replaces: detect_capability_command, detect_task_command, detect_email_command
        ...
        # ── Web search ──
        # Runs search, injects results as context, lets Hayeong synthesize naturally.
        _web_context = ""
        if intent["intent"] == "web_search" and SEARCH_AVAILABLE:
            _log_search = any(kw in user_input.lower() for kw in ["news", "latest", "current"])
            query = WebSearch.extract_query(user_input)
            ...
            if LOGGER_AVAILABLE:
                logger.log_capability_used(
                    ...
                    details={"query": query, "results": len(data.get("results", []))}
                )

        # ── Vision ──
        # Takes screenshot (or reads image), injects description as context,
        # lets Hayeong respond naturally about what she sees.
        _vision_context = ""
        if intent["intent"] == "vision" and VISION_AVAILABLE:
            u = user_input.lower()
            if any(x in u for x in ["deep", "detail", "explain", "what's in", "code"]):
                _speak("Let me take a closer look.", emotion="neutral")
                _vision_context = vision.look_at_screen_deep(user_input)
            elif any(x in u for x in ["image", "photo", "file", "this image", "this photo"]):
