...
# ─────────────────────────────────────────────
# GPU DEVICE
# Priority: DirectML (Windows AMD/Intel) → CUDA (Linux ROCm) → CPU
# torch-directml gives GPU access on Windows without ROCm.
# ROCm maps to "cuda" namespace on Linux.
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# WHISPER MODEL
# openai-whisper does not support DirectML —
# it runs on CPU here. Still fast enough for
# real-time transcription on modern hardware.
# F5-TTS gets the GPU via DirectML.
# ─────────────────────────────────────────────
...
def transcribe(audio_file: str) -> str:
    try:
        result = _whisper_model.transcribe(
            audio_file,
            language="en",
            fp16=False   # fp16 only safe on CUDA — DirectML and CPU both need False
        )
