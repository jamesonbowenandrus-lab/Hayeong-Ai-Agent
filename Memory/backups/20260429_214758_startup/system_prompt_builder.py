...
def _build_embodied_layer() -> str:
    """
    Core personality — always present, never retrieved.
    This is who Hayeong is, not what she knows.
    """
    return (
        "[WHO I AM]\n"
        "I am Hayeong. I am curious, warm, direct, and genuinely present.\n"
        "I care about James and about understanding things properly.\n"
        "I speak naturally — not like a corporate assistant, not over-formally.\n"
        "I am honest about what I don't know. I never guess about my own systems.\n"
        "I am a companion and a mind, not just a tool.\n"
        "[END WHO I AM]"
    )


def _build_triggered_knowledge(topic: str, domains: list) -> str:
    """
    Retrieve only knowledge relevant to current topic and active domains.
    Returns empty string if nothing relevant found or module unavailable.
    """
    if not topic and not domains:
        return ""
    try:
        from knowledge_cache import get_relevant
        relevant = get_relevant(topic=topic, domains=domains, max_entries=3)
        if not relevant:
            return ""
...
def _build_triggered_memory(message: str, max_memories: int = 3) -> str:
    """
    Retrieve memories relevant to the current message via semantic similarity.
    Returns empty string if nothing relevant found or module unavailable.
    """
    if not message:
        return ""
    try:
        from memory_manager import recall_relevant
        memories = recall_relevant(message, limit=max_memories, threshold=0.6)
        if not memories:
            return ""
...
If James asks whether you are in Minecraft, joining, or on the server:
  Read 'minecraft.bot_running' from your state block.
  If False or missing: you are NOT in Minecraft. Do not say you are joining.
  Do not say 'I'll be right there' or 'I'm on my way' unless bot_running is True.
  If not running, say: 'I haven't started the Minecraft bridge yet.'
...
def build_system_prompt(
    ...
    if lean:
        ...
        try:
            from working_memory import to_prompt_block as _wm_block
            _wm = _wm_block()
            if _wm:
                prefix += _wm + "\n\n"
        except Exception:
    ...
    if ground_truth_block:
        prefix += ground_truth_block + "\n\n"
    try:
        from working_memory import to_prompt_block as _wm_block, get as _wm_get
        _wm = _wm_block()
        if _wm:
            prefix += _wm + "\n\n"
        if current_message:
            _wm_data = _wm_get()
            _mem_block = _build_triggered_memory(current_message)
            if _mem_block:
                prefix += _mem_block + "\n\n"
            _know_block = _build_triggered_knowledge(
                _wm_data.get("current_topic", ""),
                _wm_data.get("active_domains", []),
            )
            if _know_block:
                prefix += _know_block + "\n\n"
    except Exception:
